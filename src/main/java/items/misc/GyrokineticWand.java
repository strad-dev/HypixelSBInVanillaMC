...
public class GyrokineticWand implements AbilityItem {
	...
	public static ItemStack getItem() {
		...
		data.displayName(Utils.mm("<dark_purple>Gyrokinetic Wand"));
		...
		lore.add(Utils.mm("<gray>location, pulling all mobs together."));
	...
	public boolean onLeftClick(Player p) {
		...
		for(double angle = 0; angle < Math.PI * 2; angle += Math.PI / 32) {
			...
			FallingBlock block = rift.getWorld().spawn(blockLoc, FallingBlock.class, fb -> fb.setBlockData(blockType.createBlockData()));
			...
			public void run() {
				...
				for(FallingBlock block : fallingBlocks) {
					if(block.isDead() || !block.isValid()) {
						...
						FallingBlock newBlock = respawnLoc.getWorld().spawn(respawnLoc, FallingBlock.class, fb -> fb.setBlockData(blockType.createBlockData()));
